...
def main_menu(screen):
    ...
    while True:
        ...
        screen.blit(font_big.render("SNAKE", True, BLUE_TITLE), (W//2 - 80, 50))
        screen.blit(font_med.render("Username:", True, TEXT_BLACK), (W//2 - 100, 130))
        ...
        screen.blit(font_med.render(name + "|", True, INPUT_TEXT_COLOR), (box.x + 6, box.y + 5))
...
def leaderboard_screen(screen):
    ...
    while True:
        screen.fill(BACKGROUND_WHITE)
        screen.blit(font_big.render("TOP 10", True, BLUE_TITLE), (W//2 - 90, 20))
        header = font_small.render("Rank  Name             Score  Lvl  Date", True, TEXT_BLACK)
        screen.blit(header, (30, 80))
        for i, (uname, score, lvl, date) in enumerate(rows):
            color = BLUE_TITLE if i == 0 else TEXT_BLACK
...
def settings_screen(screen):
    ...
    while True:
        screen.fill(BACKGROUND_WHITE)
        screen.blit(font_big.render("Settings", True, BLUE_TITLE), (W//2 - 110, 30))

        cur_color = s["snake_color"]
        cidx = color_opts.index(cur_color) if cur_color in color_opts else 0
        screen.blit(font_med.render(f"Snake color: {color_names[cidx]}", True, TEXT_BLACK), (30, 130))
        pygame.draw.rect(screen, cur_color, pygame.Rect(W-80, 128, 40, 30))
        b_col = btn(screen, "Change", W-220, 128, w=130, h=36)

        screen.blit(font_med.render(f"Grid: {'ON' if s['grid'] else 'OFF'}", True, TEXT_BLACK), (30, 200))
        b_grid = btn(screen, "Toggle", W-220, 198, w=130, h=36)

        screen.blit(font_med.render(f"Sound: {'ON' if s['sound'] else 'OFF'}", True, TEXT_BLACK), (30, 270))
...
def game_over_screen(screen, score, level, personal_best):
    ...
    while True:
        ...
        screen.blit(font_med.render(f"Score: {score}",          True, TEXT_BLACK), (W//2-100, 200))
        screen.blit(font_med.render(f"Level: {level}",          True, TEXT_BLACK), (W//2-100, 245))
        screen.blit(font_med.render(f"Best:  {personal_best}",  True, BLUE_TITLE), (W//2-100, 290))
